Feature: Verify end to end flow dominos Order one veg pizza without login
  ...
  @RegressionTest @SmokeTest @DominosOrderOnlineTest @RerunTest
  Scenario Outline: Verify End To End flow to order one veg pizza without login1
    ...
    And Enter and Select Area as "&&&&&()*(*()*Utsav ResidencyascacacvVCSC scs v"
    Examples: 
      | Section   | Product    | Quantity |
      | Veg Pizza | Margherita |        1 |

  @RegressionTest @SmokeTest @DominosOrderOnlineTest @RerunTest
  Scenario Outline: Verify End To End flow to order one veg pizza without login2
    Given Verify user is on Dominos Home page
    When Click on the order online button
    Then Verify contact delivery page
    When Click on locate me button
